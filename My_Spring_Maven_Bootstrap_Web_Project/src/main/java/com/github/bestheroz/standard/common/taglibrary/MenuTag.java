...
public class MenuTag extends TagSupport {
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            if (MySessionUtils.getAttribute(session, MENU_TAG) == null) {
                final Document doc = Jsoup.parseBodyFragment(IOUtils.toString(new File(session.getServletContext().getRealPath("/WEB-INF/views/common/menu.html")).toURI(), StandardCharsets.UTF_8));
                ...
                MySessionUtils.setAttribute(session, MENU_TAG, doc.body().html());
            }
    ...
    private void drawMenuData(final Document doc, final JsonObject param) throws JspException {
        final List<MenuVO> menuList = MyAccessBeanUtils.getBean(MenuService.class).getMenuVOList(param);
